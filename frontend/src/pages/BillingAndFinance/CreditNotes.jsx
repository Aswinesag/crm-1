...
const CreditNotes = () => {
  ...
  const [creditNotes, setCreditNotes] = useState([
    ...
    },
    {
      ...
      reason: "Price correction - Overcharged",
      subtotal: 5000,
      cgst: 450,
      sgst: 450,
      igst: 0,
  ...
  const filteredCreditNotes = creditNotes.filter((creditNote) => {
    const matchesSearch =
      creditNote.creditNoteNumber.toLowerCase().includes(searchQuery.toLowerCase()) ||
      creditNote.customerName.toLowerCase().includes(searchQuery.toLowerCase()) ||
      creditNote.invoiceNumber.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesFilter = statusFilter ? creditNote.status === statusFilter : true;
    return matchesSearch && matchesFilter;
  });

  const totalCreditNotes = creditNotes.length;
  const approvedCreditNotes = creditNotes.filter((cn) => cn.status === "Approved").length;
  const pendingCreditNotes = creditNotes.filter((cn) => cn.status === "Pending").length;
  const rejectedCreditNotes = creditNotes.filter((cn) => cn.status === "Rejected").length;
  ...
  const handleStatusChange = (id, newStatus) => {
    setCreditNotes(creditNotes.map((cn) => 
      cn.id === id ? { ...cn, status: newStatus } : cn
    ));
  ...
  return (
    <div className="mt-4">
      ...
      <div>
        ...
        <span className="text-[#C2410C]"> Credit Notes </span>
      </div>
      ...
      <div className="flex justify-center text-center md:text-start flex-wrap gap-3 xl:gap-4 mt-4">
        ...
        <Card
          title="Approved"
          count={approvedCreditNotes}
          bg="#EAF1FA"
          color="#1C4CD2"
        />
        <Card
          title="Pending"
          count={pendingCreditNotes}
          bg="#FAF5FF"
          color="#7E22CE"
        />
        <Card
          title="Rejected"
          count={rejectedCreditNotes}
          bg="#FAF0F0"
          color="#BA1D1D"
        />
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-2 mb-6 mt-6">
        ...
        <div className="flex gap-2 flex-col xl:flex-row items-center">
          ...
          >
            <FiPlus />
            <button className="cursor-pointer">Create Credit Note</button>
          </div>
        </div>
      </div>
          ...
          <tbody>
            {filteredCreditNotes.length > 0 ? (
              filteredCreditNotes.map((creditNote) => (
                <tr key={creditNote.id} className="hover:bg-gray-50 bg-white">
                  ...
                  <td className="p-3 border-b border-gray-300 font-medium text-red-600">
                    -₹{creditNote.grandTotal.toLocaleString()}
                  </td>
                  <td className="p-3 border-b border-gray-300">
                    <select
        ...
        <div className="fixed inset-0 bg-transparent backdrop-blur-md bg-opacity-70 flex justify-center items-center z-50 overflow-y-auto">
          <div className="bg-white p-6 rounded-md max-w-6xl w-full my-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold">Create New Credit Note</h2>
              <button
            ...
            <form onSubmit={handleCreateCreditNote}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div>
                  <label className="block mb-1 font-medium text-sm">Credit Note Number</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block mb-1 font-medium text-sm">Credit Note Date</label>
                  <input
                    type="date"
                    name="creditNoteDate"
              ...
              <div className="mb-6">
                <label className="block mb-1 font-medium text-sm">Reason for Credit Note</label>
                <textarea
                  name="reason"
                  ...
                  className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:border-[#FB6514]"
                  rows="3"
                  placeholder="Enter reason for issuing credit note (e.g., Goods returned, Price correction, etc.)"
                  required
                />
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-md mb-6">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div className="flex justify-between">
                    ...
                    <span className="text-red-600">-₹{calculateTotals().subtotal.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium">CGST:</span>
                    <span className="text-red-600">-₹{calculateTotals().cgst.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium">SGST:</span>
                    <span className="text-red-600">-₹{calculateTotals().sgst.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium">IGST:</span>
                    <span className="text-red-600">-₹{calculateTotals().igst.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between font-bold text-lg md:col-span-2">
                    <span>Total Credit Amount:</span>
                    <span className="text-red-600">-₹{calculateTotals().grandTotal.toLocaleString()}</span>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white p-6 rounded-md max-w-4xl w-full my-8">
            ...
            <div className="space-y-4">
              <div className="bg-red-50 border border-red-200 p-4 rounded-md mb-4">
                <p className="text-sm text-red-600 font-medium">Reason for Credit Note:</p>
                <p className="text-red-800">{selectedCreditNote.reason}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 border-b pb-4">
                ...
                <div>
                  <p className="text-sm text-gray-500">Credit Note Date</p>
                  <p className="font-medium">{selectedCreditNote.creditNoteDate}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedCreditNote.invoiceDate}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 border-b pb-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedCreditNote.gstin}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-sm text-gray-500">Billing Address</p>
                  <p className="font-medium">{selectedCreditNote.billingAddress}</p>
                </div>
              </div>
                  ...
                  <tbody>
                    {selectedCreditNote.items.map((item, index) => (
                      <tr key={index} className="border-b">
                        ...
                        <td className="p-2 text-right text-red-600">-₹{item.amount.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="bg-red-50 p-4 rounded-md">
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span>Subtotal:</span>
                    <span className="text-red-600">-₹{selectedCreditNote.subtotal.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>CGST:</span>
                    <span className="text-red-600">-₹{selectedCreditNote.cgst.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>SGST:</span>
                    <span className="text-red-600">-₹{selectedCreditNote.sgst.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>IGST:</span>
                    <span className="text-red-600">-₹{selectedCreditNote.igst.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between font-bold text-lg border-t pt-2">
                    <span>Total Credit Amount:</span>
                    <span className="text-red-600">-₹{selectedCreditNote.grandTotal.toLocaleString()}</span>
                  </div>
                </div>
              </div>

...
const PurchaseOrderDetails = () => {
  ...
  return <div className="p-6 space-y-6"><div className="flex flex-wrap justify-between gap-3"><div><h1 className="text-3xl font-bold">{po.poNumber}</h1><p>{po.vendorId?.vendorName} · {po.warehouseId?.warehouseName || "Warehouse selected at receipt"}</p>{po.sourceRFQ && <p className="text-sm opacity-70">Source {po.sourceRequisition?.requisitionNo} → {po.sourceRFQ?.rfqNumber}</p>}</div><div className="flex flex-wrap gap-2 items-center"><span className="badge badge-lg">{po.status}</span>{!["RECEIVED", "CANCELLED"].includes(po.status) && <Link className="btn btn-primary btn-sm" to={`/procurement/grns/create?purchaseOrder=${po._id}`}>Create GRN</Link>}{!["DRAFT", "CANCELLED"].includes(po.status) && <Link className="btn btn-outline btn-sm" to={`/procurement/purchase-bills/new?purchaseOrder=${po._id}`}>Create Purchase Bill</Link>}</div></div><div className="overflow-x-auto bg-base-100 rounded-xl shadow"><table className="table table-zebra"><thead><tr><th>Item</th><th>Type</th><th>Ordered</th><th>Received</th><th>Pending</th><th>Unit price</th></tr></thead><tbody>{po.items.map((line) => <tr key={line._id}><td>{line.itemNameSnapshot || line.item?.productName || line.item?.materialName || line.item?.componentName}</td><td>{line.itemType}</td><td>{line.quantity}</td><td>{line.receivedQuantity}</td><td>{line.pendingQuantity}</td><td>₹{Number(line.unitPrice || 0).toLocaleString()}</td></tr>)}</tbody></table></div><div className="stat bg-base-100 rounded-xl shadow"><div className="stat-title">Total ordered value</div><div className="stat-value text-2xl">₹{total.toLocaleString()}</div></div></div>;
};
